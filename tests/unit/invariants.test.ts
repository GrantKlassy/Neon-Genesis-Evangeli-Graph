...
describe("Invariant: 'child' tag is restricted to characters who pilot an EVA", () => {
  ...
  // Toji (4th), Kaworu (5th). These are canon-locked --- adding a sixth
  // hypothetical Child would be a Rebuild leak and is out of scope; this
  // list defines the universe of valid 'child' tag carriers.
  const REQUIRED_CHILDREN: ReadonlyArray<string> = [
    "char_shinji",
    "char_rei",
    "char_asuka",
    "char_toji",
  ...
  it("happy path: every required Child carries the 'child' tag (subset, no equality lock)", () => {
    // Subset rather than equality: the test fails if a required Child
    // loses the tag, but does NOT fail if some future supporting child
    // (e.g. a flashback addition) gets the tag too. The 'child only on
    // characters' invariant below catches abuse of the tag.
    ...
    for (const id of REQUIRED_CHILDREN) {
      expect(tagged.has(id), `required Child ${id} missing the 'child' tag`).toBe(
        true,
      );
    }
  });
  ...
  it("happy path: every 'child' character EXCEPT Kaworu has a pilots edge to an EVA", () => {
    ...
    const requirePilots = REQUIRED_CHILDREN.filter((id) => id !== "char_kaworu");
...
describe("Invariant: visibleTags spoiler logic", () => {
  ...
  it("a fully-spoiled viewer sees every dies-by-end-of-series tag (count-derived, no threshold)", () => {
    // Structural: at full progress, the count of VISIBLE death tags equals
    // the count of AUTHORED death tags. Adding/removing a death-tag in the
    // dataset never breaks this test --- it derives the expected count
    // from the dataset itself instead of locking a threshold.
    let visibleDeaths = 0;
    let authoredDeaths = 0;
    for (const node of evangelion.nodes) {
      if (!node.tags) continue;
      for (const t of node.tags) {
        if (t.id === "dies-by-end-of-series") authoredDeaths++;
      }
    ...
    expect(authoredDeaths).toBeGreaterThan(0);
    expect(visibleDeaths).toBe(authoredDeaths);
  });
