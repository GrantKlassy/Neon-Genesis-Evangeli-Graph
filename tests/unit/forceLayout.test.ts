...
describe("forceLayout3D", () => {
  ...
  it("pins the requested node at the origin and recenters around it", () => {
    // The renderer pins Shinji so the orb's literal axis of rotation
    // becomes him. Assert the invariant directly: pinned node ends at
    // (0,0,0) every iteration count, regardless of how the rest of the
    // graph distributes itself.
    for (const iters of [1, 100, 600]) {
      const { positions } = forceLayout3D(
        evangelion.nodes,
        evangelion.edges,
        {
      ...
      const p = positions.get("char_shinji")!;
      expect(p.x).toBe(0);
      expect(p.y).toBe(0);
      expect(p.z).toBe(0);
    }
  ...
  it("with Shinji pinned, the average node-to-Shinji distance shrinks vs. unpinned baseline", () => {
    // "Shinji is the most magnetic" means the renderer's pinned + bumped
    // centerForce config draws the cloud tighter around him than the
    // default centroid-recenter, weak-gravity layout would. Assert the
    // tightening end-to-end with the same parameters the renderer uses,
    // against a baseline that uses neither pin nor boost.
    const baseline = forceLayout3D(evangelion.nodes, evangelion.edges, {
      iterations: 600,
      springLengthByKind: EDGE_SPRING_LENGTH,
      seed: 0xc0ffee,
    });
    const magnetized = forceLayout3D(evangelion.nodes, evangelion.edges, {
      iterations: 600,
      springLengthByKind: EDGE_SPRING_LENGTH,
      pinnedNodeId: "char_shinji",
      centerForce: 0.06,
    ...
    const distFromShinji = (
      positions: Map<string, { x: number; y: number; z: number }>,
    ) => {
      const s = positions.get("char_shinji")!;
      let sum = 0;
      let n = 0;
      for (const [id, p] of positions) {
        if (id === "char_shinji") continue;
        sum += Math.sqrt(
    ...
    expect(distFromShinji(magnetized.positions)).toBeLessThan(
      distFromShinji(baseline.positions),
    );
  });
