...
function buildEdges(): Edge[] {
  ...
  // Mixed-kind connection table. Each entry defaults to `generic` (the
  // catch-all for "these two are related, no canonical relation type fits")
  // but may declare a `kind` to claim one of the derived typed classes:
  //   - relationship: character <-> character interpersonal bonds.
  //   - afflicts:     a psych-wound concept -> the character who carries it.
  //   - attacked:     an Angel -> the site / unit / pilot it struck.
  //   - manifests:    the A.T. Field -> the Angel / Eva / human projecting one.
  // Entries with no `kind` stay generic --- equipment mechanics, scheming
  // ties, character signature objects, and other one-off relations that
  // don't cleanly fit a typed class. The push loop below stamps each entry
  // with its kind's spring weight.
  const supportEdges: Array<{
    ...
    { from: "char_pen_pen", to: "char_misato", kind: "relationship", shortcodes: ["penPen", "misato"], notes: "Pen Pen lives in Misato's apartment." },
    { from: "char_hikari", to: "char_asuka", kind: "relationship", shortcodes: ["hikari", "asuka"], notes: "Class rep --- Asuka's best friend.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "Inherits Asuka's Ep. 8 first-appearance gate" },
    { from: "char_hikari", to: "char_toji", kind: "relationship", shortcodes: ["hikari", "toji"], notes: "Class rep --- not-so-secret crush on Toji.", revealedAt: { kind: "ep", episode: 3 }, revealedAtSource: "Inherits Hikari + Toji Ep. 3 introduction" },
    { from: "char_kensuke", to: "char_shinji", kind: "relationship", shortcodes: ["kensuke", "shinji"], notes: "Classmate. Camera glued to his hand." },
    { from: "char_kensuke", to: "char_toji", kind: "relationship", shortcodes: ["kensuke", "toji"], notes: "Classmate.", revealedAt: { kind: "ep", episode: 3 }, revealedAtSource: "Inherits Kensuke + Toji Ep. 3 introduction" },
    { from: "char_misato", to: "char_kaji", kind: "relationship", shortcodes: ["misato", "kaji"], notes: "Misato and Kaji --- exes from college.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/Ryoji_Kaji --- Ep. 8 reintroduction reveals their college relationship" },
    { from: "char_kaji", to: "char_asuka", kind: "relationship", shortcodes: ["kaji", "asuka"], notes: "Asuka's escort on the Pacific fleet.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/Ryoji_Kaji --- 'accompanies Asuka from Germany to Tokyo-3 during Ep. 8'" },
    { from: "char_fuyutsuki", to: "char_yui", kind: "relationship", shortcodes: ["fuyutsuki", "yui"], notes: "Yui Ikari was Fuyutsuki's metaphysical-biology student.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/Episode_21 --- Director's Cut backstory drop establishes Fuyutsuki/Yui mentor-student relationship" },
    ...
    // belong on the graph.
    { from: "char_misato", to: "char_shinji", kind: "relationship", shortcodes: ["misato", "shinji"], notes: "Misato is Shinji's guardian --- he moves into her apartment in Ep. 2.", revealedAt: { kind: "ep", episode: 2 }, revealedAtSource: "https://wiki.evageeks.org/Misato_Katsuragi --- Shinji moves into the Katsuragi apartment in Ep. 2 ('The Beast')" },
    { from: "char_misato", to: "char_asuka", kind: "relationship", shortcodes: ["misato", "asuka"], notes: "Asuka moves into Misato's apartment in Ep. 9 ('Both of you, dance!').", revealedAt: { kind: "ep", episode: 9 }, revealedAtSource: "https://wiki.evageeks.org/Episode_09 --- Asuka moves into the Katsuragi apartment for the synchronized-dance training arc" },
    ...
    { from: "char_maya", to: "char_ritsuko", kind: "relationship", shortcodes: ["maya", "ritsuko"], notes: "Maya adores Ritsuko --- her structural anchor on the bridge." },

    // Hyuga's quiet crush on Misato --- the recurring bridge-crew running
    // beat. Open from Ep. 1 alongside the bridge introductions.
    { from: "char_hyuga", to: "char_misato", kind: "relationship", shortcodes: ["hyuga", "misato"], notes: "Hyuga carries a quiet crush on Misato across every bridge scene." },
    ...
    { from: "char_gendo", to: "char_rei", kind: "relationship", shortcodes: ["gendo", "rei"], notes: "Gendo's pseudo-paternal bond with Rei --- the one person he treats with tenderness.", revealedAt: { kind: "ep", episode: 5 }, revealedAtSource: "https://wiki.evageeks.org/Rei_Ayanami --- the Gendo-Rei dynamic surfaces during the Ep. 5/6 Ramiel arc" },
    ...
    { from: "concept_rejection", to: "char_shinji", kind: "afflicts", shortcodes: ["rejection", "shinji"], notes: "Gendo abandoned Shinji at Yui's death and has rejected him every interaction since. The defining wound." },
    { from: "concept_rejection", to: "char_asuka", kind: "afflicts", shortcodes: ["rejection", "asuka"], notes: "Kyoko mistook a doll for Asuka before her suicide. Asuka's overcompensating bravado is rejection-armor.", revealedAt: { kind: "ep", episode: 22 }, revealedAtSource: "https://wiki.evageeks.org/Arael --- Arael's Ep. 22 mind-attack pries open Asuka's mother-rejection backstory" },
    { from: "concept_rejection", to: "char_ritsuko", kind: "afflicts", shortcodes: ["rejection", "ritsuko"], notes: "Ritsuko realizes Gendo has used and discarded her --- 'I've been discarded' (Ep. 23, 'Rei III').", revealedAt: { kind: "ep", episode: 23 }, revealedAtSource: "https://wiki.evageeks.org/Ritsuko_Akagi --- the 'I've been discarded' confrontation lands in Ep. 23" },
    { from: "concept_rejection", to: "char_rei", kind: "afflicts", shortcodes: ["rejection", "rei"], notes: "Rei III walks past Rei II's clothes in the locker --- the replacement is the rejection.", revealedAt: { kind: "ep", episode: 23 }, revealedAtSource: "https://wiki.evageeks.org/Rei_Ayanami --- Ep. 23 'Rei III' establishes the replacement chain" },

    // Abandonment --- parents who left or were taken. The through-line
    // the show keeps naming.
    { from: "concept_abandonment", to: "char_shinji", kind: "afflicts", shortcodes: ["abandonment", "shinji"], notes: "Gendo placed Shinji with a teacher after Yui's death and did not see him again until summoning him to pilot Unit-01.", revealedAt: { kind: "ep", episode: 1 }, revealedAtSource: "https://wiki.evageeks.org/Episode_01 --- the 'sent away after Mom died' backstory is dropped in the opening episode" },
    { from: "concept_abandonment", to: "char_asuka", kind: "afflicts", shortcodes: ["abandonment", "asuka"], notes: "Kyoko Zeppelin Soryu's psychotic break and suicide left Asuka raised by stepmother and EVA program.", revealedAt: { kind: "ep", episode: 22 }, revealedAtSource: "https://wiki.evageeks.org/Arael --- the Kyoko-suicide backstory surfaces during Arael's Ep. 22 mind-attack" },
    { from: "concept_abandonment", to: "char_misato", kind: "afflicts", shortcodes: ["abandonment", "misato"], notes: "Misato's father sealed her in an escape pod and died at Antarctica. The grief drives her drinking and the Kaji push-pull.", revealedAt: { kind: "ep", episode: 12 }, revealedAtSource: "https://wiki.evageeks.org/Misato_Katsuragi --- her father's loss surfaces in the Ep. 12 Sahaquiel breakdown" },
    { from: "concept_abandonment", to: "char_rei", kind: "afflicts", shortcodes: ["abandonment", "rei"], notes: "Rei lives alone in a barren apartment off-base. No parents, no family, no home --- abandonment as default state.", revealedAt: { kind: "ep", episode: 5 }, revealedAtSource: "https://wiki.evageeks.org/Rei_Ayanami --- Rei's solitary apartment is established in Ep. 5" },
    { from: "concept_abandonment", to: "char_ritsuko", kind: "afflicts", shortcodes: ["abandonment", "ritsuko"], notes: "Naoko's suicide off the Magi catwalk left Ritsuko an orphan and chasing her mother's role at NERV.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/Episode_21 --- the Naoko-suicide flashback lands in the Director's Cut backstory drop" },
    { from: "concept_abandonment", to: "char_gendo", kind: "afflicts", shortcodes: ["abandonment", "gendo"], notes: "Yui's loss to Unit-01 is the wound Gendo organizes the entire Instrumentality scenario around. He runs the war to reach her.", revealedAt: { kind: "ep", episode: 20 }, revealedAtSource: "https://wiki.evageeks.org/Yui_Ikari --- the contact-experiment loss surfaces in the Ep. 20 Director's Cut backstory" },
    ...
    { from: "concept_trauma", to: "char_shinji", kind: "afflicts", shortcodes: ["trauma", "shinji"], notes: "Watched Unit-01 swallow his mother at age four. Carries it into every entry plug." },
    { from: "concept_trauma", to: "char_asuka", kind: "afflicts", shortcodes: ["trauma", "asuka"], notes: "The doll. The hospital. The mother who didn't recognize her. Asuka's whole sync ratio collapses around it in Ep. 22.", revealedAt: { kind: "ep", episode: 22 }, revealedAtSource: "https://wiki.evageeks.org/Arael --- the Kyoko trauma surfaces under Arael's mind-attack (Ep. 22)" },
    { from: "concept_trauma", to: "char_rei", kind: "afflicts", shortcodes: ["trauma", "rei"], notes: "Strangled by Naoko in 2010, replaced after each death --- the trauma is being replaceable.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/Episode_21 --- Naoko strangling Rei is the Director's Cut backstory drop" },
    { from: "concept_trauma", to: "char_misato", kind: "afflicts", shortcodes: ["trauma", "misato"], notes: "Sole survivor of the Katsuragi Expedition. Mute for years after Second Impact. The Yebisu and the Kaji are the painkiller.", revealedAt: { kind: "ep", episode: 12 }, revealedAtSource: "https://wiki.evageeks.org/Misato_Katsuragi --- the Antarctica survival trauma surfaces in Ep. 12" },
    { from: "concept_trauma", to: "char_ritsuko", kind: "afflicts", shortcodes: ["trauma", "ritsuko"], notes: "Mother's suicide, mother's affair, the role she stepped into. Ritsuko inherits Naoko's place in Gendo's bed and at the Magi terminal.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/Ritsuko_Akagi --- the Naoko backstory surfaces in Ep. 21" },
    { from: "concept_trauma", to: "char_gendo", kind: "afflicts", shortcodes: ["trauma", "gendo"], notes: "The contact experiment took Yui whole while he watched. Every gloved-hand silence after is the same room.", revealedAt: { kind: "ep", episode: 20 }, revealedAtSource: "https://wiki.evageeks.org/Yui_Ikari --- Gendo's witnessing of the contact experiment surfaces in Ep. 20" },
    ...
    { from: "concept_hedgehogs_dilemma", to: "char_shinji", kind: "afflicts", shortcodes: ["hedgehogsDilemma", "shinji"], notes: "The named subject of Ritsuko's Ep. 4 dialogue. Every approach toward intimacy ends in retreat.", revealedAt: { kind: "ep", episode: 4 }, revealedAtSource: "https://wiki.evageeks.org/Hedgehog_Dilemma --- named on screen in Ep. 4 about Shinji" },
    { from: "concept_hedgehogs_dilemma", to: "char_asuka", kind: "afflicts", shortcodes: ["hedgehogsDilemma", "asuka"], notes: "Pushes everyone away with rage; demands closeness with the same breath.", revealedAt: { kind: "ep", episode: 9 }, revealedAtSource: "https://wiki.evageeks.org/Episode_09 --- Asuka's push-pull lands in the Ep. 9 dance-training arc" },
    { from: "concept_hedgehogs_dilemma", to: "char_misato", kind: "afflicts", shortcodes: ["hedgehogsDilemma", "misato"], notes: "Ritsuko frames the dilemma about Misato in the Ep. 4 dialogue --- the Kaji push-pull is the adult version.", revealedAt: { kind: "ep", episode: 4 }, revealedAtSource: "https://wiki.evageeks.org/Hedgehog_Dilemma --- the Ep. 4 framing applies to Misato as well as Shinji" },
    ...
    { from: "concept_depression", to: "char_shinji", kind: "afflicts", shortcodes: ["depression", "shinji"], notes: "The defining carrier. 'I mustn't run away' as a depressive command, Ep. 16 acceptance of being eaten, Ep. 24 bathwater stillness, Ep. 25-26 surrender." },
    ...
    { from: "char_asuka", to: "char_kyoko", kind: "relationship", shortcodes: ["asuka", "kyoko"], notes: "Asuka's mother --- the doll, the hospital, the 'die with me.' Every Asuka collapse is this wound resurfacing.", revealedAt: { kind: "ep", episode: 22 }, revealedAtSource: "Inherits Kyoko's Ep. 22 first-surfacing gate" },
    ...
    { from: "concept_at_field", to: "eva_unit01", kind: "manifests", shortcodes: ["atField", "unit01"], notes: "Evangelions project an A.T. Field of their own --- the same barrier the Angels manifest, strong enough to erode an Angel's field on contact and punch through. (wiki/A.T._Field)" },
    { from: "concept_at_field", to: "angel_05_ramiel", kind: "manifests", shortcodes: ["atField", "ramiel"], notes: "Ramiel carries one of the strongest A.T. Fields of any Angel --- 180 million kilowatts were needed to punch through it at Operation Yashima.", revealedAt: { kind: "ep", episode: 5 }, revealedAtSource: "https://wiki.evageeks.org/Ramiel --- 'one of the strongest A.T. Fields of all the Angels'" },
    ...
    { from: "char_gendo", to: "char_shinji", kind: "relationship", shortcodes: ["gendo", "shinji"], notes: "Father and son. Gendo summoned Shinji to Tokyo-3 the day NERV needed a Unit-01 pilot --- the first time he had addressed him in years. The wound the whole show turns on." },
    { from: "char_shinji", to: "char_rei", kind: "relationship", shortcodes: ["shinji", "rei"], notes: "Shinji and the First Child. He carries the broken-and-bandaged Rei off her gurney, refuses to let her be the spare, and slowly thaws her affect --- the bond that teaches him people are reachable." },
    { from: "char_shinji", to: "char_asuka", kind: "relationship", shortcodes: ["shinji", "asuka"], notes: "The central teen dynamic --- co-pilots, flatmates, rivals, the Ep. 15 kiss 'to kill time.' Every approach toward each other ends in retreat.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/Asuka_Langley_Soryu --- Asuka arrives in Ep. 8 and is folded into Shinji's life from there" },
    { from: "char_shinji", to: "char_kaworu", kind: "relationship", shortcodes: ["shinji", "kaworu"], notes: "The Fifth Child befriends Shinji in a single episode --- shares a bath, says 'suki da,' and is the first person to tell Shinji he is worthy of love, before asking to be killed.", revealedAt: { kind: "ep", episode: 24 }, revealedAtSource: "https://wiki.evageeks.org/Kaworu_Nagisa --- Kaworu tells Shinji he is 'favorable' (suki) in Ep. 24" },
    { from: "char_shinji", to: "char_toji", kind: "relationship", shortcodes: ["shinji", "toji"], notes: "Classmate turned friend. Toji swings a punch at Shinji over his hospitalized sister, then asks Shinji to hit him back after the Shamshel sortie --- and later pilots the doomed Unit-03.", revealedAt: { kind: "ep", episode: 3 }, revealedAtSource: "https://wiki.evageeks.org/Toji_Suzuhara --- 'Toji Suzuhara is Shinji Ikari's classmate and later friend'; the punch lands in Ep. 3" },
    { from: "char_rei", to: "char_asuka", kind: "relationship", shortcodes: ["rei", "asuka"], notes: "The other two Children. Asuka cannot stand Rei's blank obedience --- pilots 'like a doll' --- and the contrast defines them both.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/Asuka_Langley_Soryu --- Asuka and Rei share the Children roster from Ep. 8" },
    { from: "char_gendo", to: "char_fuyutsuki", kind: "relationship", shortcodes: ["gendo", "fuyutsuki"], notes: "Commander and Sub-Commander. Fuyutsuki has stood at Gendo's shoulder since the GEHIRN days --- the one man who knew Yui and still disagrees with Gendo out loud." },
    { from: "char_misato", to: "char_ritsuko", kind: "relationship", shortcodes: ["misato", "ritsuko"], notes: "College friends from Tokyo-2 (where Misato was dating Kaji). The bridge's operations/science double act, drifting apart as Ritsuko keeps NERV's secrets. (wiki/Ritsuko_Akagi)" },
    { from: "char_ritsuko", to: "char_kaji", kind: "relationship", shortcodes: ["ritsuko", "kaji"], notes: "Ritsuko knew Kaji at Tokyo-2 college as Misato's boyfriend --- the third corner of the university triangle.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/Ritsuko_Akagi --- 'Ritsuko met Misato Katsuragi in college at Tokyo-2, as well as Misato's boyfriend Ryoji Kaji'" },
    { from: "char_kaji", to: "char_gendo", kind: "relationship", shortcodes: ["kaji", "gendo"], notes: "Kaji answers to Gendo as NERV's special inspector --- and personally hand-delivers the embryonic Adam to him, the courier job that gets him killed.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/Ryoji_Kaji --- Kaji rejoins NERV as a special inspector under Gendo from Ep. 8" },
    { from: "char_ritsuko", to: "char_rei", kind: "relationship", shortcodes: ["ritsuko", "rei"], notes: "Ritsuko maintains the Rei clones --- and in Ep. 23 destroys the Dummy Plant tank of spare Reis, insisting what she really killed was Rei, her rival.", revealedAt: { kind: "ep", episode: 23 }, revealedAtSource: "https://wiki.evageeks.org/Ritsuko_Akagi --- 'Ritsuko destroyed the Dummy System and with it the mindless Rei clones'" },
    ...
    { from: "char_keel", to: "char_gendo", kind: "relationship", shortcodes: ["keel", "gendo"], notes: "Keel chairs the committee Gendo answers to --- SEELE-01 over the NERV Commander, until Gendo stops taking the calls late in the show.", revealedAt: { kind: "ep", episode: 14 }, revealedAtSource: "https://wiki.evageeks.org/Keel_Lorenz --- Keel is the SEELE chairman directing Gendo's chain of command (first on screen Ep. 14)" },

    // =====================================================================
    // Canon-expansion pass (2026-06-15): 50 additional generic ties surfaced
    // by another EvaGeeks pass. Each edge is gated to >= the later of its two
    // endpoint gates (monotonicity) and cites the page it was authored
    // against where gated. Themes: the A.T. Field as a universal barrier
    // (every Angel and EVA manifests one), Angel attack target sites, the
    // Central Dogma bridge crew, the SEELE / Scrolls / Instrumentality
    // chrome, the two-Moons / Impact backstory, and the late-show Terminal
    // Dogma convergence.
    // =====================================================================

    // --- A.T. Field as the universal barrier. The seed wired it to Unit-01,
    // Ramiel, Instrumentality and the Hedgehog's Dilemma; these add the first
    // field the series ever shows (Sachiel), the prototype's own field, the
    // humanity-wide fields collapsed at Instrumentality, and the two pilots /
    // units whose field play is canon. ---
    { from: "concept_at_field", to: "angel_03_sachiel", kind: "manifests", shortcodes: ["atField", "sachiel"], notes: "Sachiel raises an octahedral A.T. Field over Tokyo-3 in the Ep. 1 cold open --- the first Absolute Terror Field the series ever shows." },
    { from: "concept_at_field", to: "eva_unit00", kind: "manifests", shortcodes: ["atField", "unit00"], notes: "Unit-00 projects its own A.T. Field --- Rei deploys it as a heat shield at Operation Yashima and as a barrier across the show." },
    { from: "concept_at_field", to: "angel_18_lilim", kind: "manifests", shortcodes: ["atField", "lilim"], notes: "Every one of the Lilim manifests a personal A.T. Field --- the wall between souls that Instrumentality collapses all at once.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/A.T._Field --- the Anti-A.T.-Field 'neutralizes all A.T. Fields across the world,' i.e. every human's own field" },
    { from: "concept_at_field", to: "eva_unit02", kind: "manifests", shortcodes: ["atField", "unit02"], notes: "Unit-02 deploys an A.T. Field of its own and erodes an Angel's field on contact --- standard Evangelion ordnance.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "https://wiki.evageeks.org/A.T._Field --- Evangelions manifest A.T. Fields like the Angels they fight" },
    { from: "concept_at_field", to: "char_rei", kind: "manifests", shortcodes: ["atField", "rei"], notes: "Rei-as-Lilith generates the Anti-A.T. Field that dissolves every barrier at Third Impact --- the field made absolute.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/A.T._Field --- the Rei/Adam/Lilith being generates 'a massive Anti A.T. Field which neutralizes all A.T. Fields across the world'" },
    { from: "concept_at_field", to: "char_kaworu", kind: "manifests", shortcodes: ["atField", "kaworu"], notes: "Kaworu controls his A.T. Field at will --- the only being who lowers it consciously, letting Unit-01 close its hand around him.", revealedAt: { kind: "ep", episode: 24 }, revealedAtSource: "https://wiki.evageeks.org/Kaworu_Nagisa --- the Seventeenth Angel manipulates his A.T. Field and drops it of his own accord (Ep. 24)" },

    // --- Angel attack target sites the seed left unwired. ---
    { from: "angel_04_shamshel", to: "loc_tokyo3", kind: "attacked", shortcodes: ["shamshel", "tokyo3"], notes: "Shamshel lashes through Tokyo-3 with its energy whips before Unit-01 closes in (Ep. 3).", revealedAt: { kind: "ep", episode: 3 }, revealedAtSource: "https://wiki.evageeks.org/Shamshel --- the Fourth Angel attacks Tokyo-3 in Ep. 3" },
    { from: "angel_05_ramiel", to: "loc_tokyo3", kind: "attacked", shortcodes: ["ramiel", "tokyo3"], notes: "Ramiel positions itself over Tokyo-3 and fires on anything that moves while it drills toward the Geofront (Ep. 5).", revealedAt: { kind: "ep", episode: 5 }, revealedAtSource: "https://wiki.evageeks.org/Ramiel --- the Fifth Angel besieges Tokyo-3 in Ep. 5" },
    { from: "angel_07_israfel", to: "loc_tokyo3", kind: "attacked", shortcodes: ["israfel", "tokyo3"], notes: "Israfel splits into two and levels swathes of Tokyo-3 before the synchronized-dance counterattack (Ep. 9).", revealedAt: { kind: "ep", episode: 9 }, revealedAtSource: "https://wiki.evageeks.org/Israfel --- the Seventh Angel attacks Tokyo-3 in Ep. 9" },
    { from: "angel_12_leliel", to: "loc_tokyo3", kind: "attacked", shortcodes: ["leliel", "tokyo3"], notes: "Leliel manifests as a vast striped sphere over Tokyo-3, its true body the shadow beneath it (Ep. 16).", revealedAt: { kind: "ep", episode: 16 }, revealedAtSource: "https://wiki.evageeks.org/Leliel --- the Twelfth Angel appears above Tokyo-3 in Ep. 16" },
    { from: "angel_14_zeruel", to: "loc_nerv_hq", kind: "attacked", shortcodes: ["zeruel", "nervHq"], notes: "Zeruel breaches NERV HQ and carves down through it toward Central Dogma, shrugging off the whole defensive line (Ep. 19).", revealedAt: { kind: "ep", episode: 19 }, revealedAtSource: "https://wiki.evageeks.org/Zeruel --- the Fourteenth Angel 'breached Nerv headquarters' in Ep. 19" },
    { from: "angel_10_sahaquiel", to: "loc_tokyo3", kind: "attacked", shortcodes: ["sahaquiel", "tokyo3"], notes: "The orbital Tenth Angel drops its whole mass onto Tokyo-3, aiming to crush the Geofront and the Adam below it (Ep. 12).", revealedAt: { kind: "ep", episode: 12 }, revealedAtSource: "https://wiki.evageeks.org/Sahaquiel --- the Tenth Angel descends on Tokyo-3 to penetrate the Geofront (Ep. 12)" },

    // --- Central Dogma: the command bridge. Misato calls the ops, Gendo and
    // Fuyutsuki preside, the bridge trio man the consoles, and the org itself
    // operates from here. Open from Ep. 1. ---
    { from: "org_nerv", to: "loc_central_dogma", shortcodes: ["nerv", "centralDogma"], notes: "Central Dogma is NERV's command bridge --- the room the entire Angel war is fought from." },
    { from: "char_misato", to: "loc_central_dogma", shortcodes: ["misato", "centralDogma"], notes: "Misato directs every Angel operation from Central Dogma as Operations Director." },
    { from: "char_gendo", to: "loc_central_dogma", shortcodes: ["gendo", "centralDogma"], notes: "Gendo presides over Central Dogma from his elevated, steepled-hands command seat." },
    { from: "char_fuyutsuki", to: "loc_central_dogma", shortcodes: ["fuyutsuki", "centralDogma"], notes: "Fuyutsuki stands at Gendo's shoulder on the Central Dogma bridge in every command scene." },
    { from: "char_maya", to: "loc_central_dogma", shortcodes: ["maya", "centralDogma"], notes: "Maya mans the sync-ratio console on the Central Dogma bridge." },
    { from: "char_hyuga", to: "loc_central_dogma", shortcodes: ["hyuga", "centralDogma"], notes: "Hyuga works the intel and sensors station on the Central Dogma bridge." },
    { from: "char_aoba", to: "loc_central_dogma", shortcodes: ["aoba", "centralDogma"], notes: "Aoba runs the sensor and comms console on the Central Dogma bridge." },

    // --- SEELE / Keel / Dead Sea Scrolls / Instrumentality chrome. ---
    { from: "char_keel", to: "concept_sound_only", shortcodes: ["keel", "soundOnly"], notes: "Keel convenes SEELE through the orange SOUND ONLY monolith --- no face, only a number and a voice.", revealedAt: { kind: "ep", episode: 14 }, revealedAtSource: "https://wiki.evageeks.org/Seele --- the committee meets via the SOUND ONLY monoliths, Keel as SEELE-01 (Ep. 14)" },
    { from: "org_seele", to: "concept_third_impact", shortcodes: ["seele", "thirdImpact"], notes: "SEELE's whole scenario drives toward a controlled Third Impact --- Instrumentality on their terms.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Human_Instrumentality_Project --- 'the forced evolution of humanity through bringing about Third Impact under their own control'" },
    { from: "char_keel", to: "concept_third_impact", shortcodes: ["keel", "thirdImpact"], notes: "Keel believes humanity is overdue to be reduced to a single soul --- Third Impact is the completion he chairs SEELE to reach.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Keel_Lorenz --- Keel drives the SEELE scenario toward Third Impact / Instrumentality" },
    { from: "org_seele", to: "event_second_impact", shortcodes: ["seele", "secondImpact"], notes: "SEELE engineered Second Impact through the Adam contact experiment and buried it under a meteorite cover story.", revealedAt: { kind: "ep", episode: 14 }, revealedAtSource: "https://wiki.evageeks.org/Second_Impact --- Second Impact was deliberately triggered under SEELE's direction and concealed" },
    { from: "concept_dead_sea_scrolls", to: "concept_human_instrumentality", shortcodes: ["deadSeaScrolls", "humanInstrumentality"], notes: "The Dead Sea Scrolls lay out the Instrumentality scenario step by step --- SEELE just executes the prophecy.", revealedAt: { kind: "ep", episode: 14 }, revealedAtSource: "https://wiki.evageeks.org/Dead_Sea_Scrolls --- the Scrolls describe the scenario that ends in Instrumentality" },
    { from: "concept_dead_sea_scrolls", to: "angel_01_adam", shortcodes: ["deadSeaScrolls", "adam"], notes: "The Scrolls predict the Angels in order and name Adam and the two Moons --- the prophecy SEELE checks every move against.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/Dead_Sea_Scrolls --- the Scrolls predict the Angels and name Adam / the Moons" },
    { from: "char_gendo", to: "concept_dead_sea_scrolls", shortcodes: ["gendo", "deadSeaScrolls"], notes: "Gendo runs NERV against SEELE's copy of the Scrolls --- following the scenario right up until he diverges from it.", revealedAt: { kind: "ep", episode: 14 }, revealedAtSource: "https://wiki.evageeks.org/Gendo_Ikari --- Gendo works from SEELE's Dead Sea Scrolls scenario before breaking with it" },
    { from: "char_gendo", to: "concept_third_impact", shortcodes: ["gendo", "thirdImpact"], notes: "Gendo intends to trigger Third Impact on his own terms --- a private Instrumentality to reach Yui, not SEELE's.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Human_Instrumentality_Project --- 'he wishes to use Instrumentality to reunite with his wife Yui'" },
    { from: "concept_human_instrumentality", to: "concept_lcl", shortcodes: ["humanInstrumentality", "lcl"], notes: "Instrumentality dissolves every person back into LCL --- the Tang all souls return to once the A.T. Fields fall.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Human_Instrumentality_Project --- humanity reverts to LCL when the barriers collapse" },

    // --- EVA / concept mechanics: the entry plug is standard kit, Unit-01's
    // berserk is Yui-in-the-machine, the Dummy Plug is Gendo's lever, and the
    // S2 organ is the Angels' shared power core. ---
    { from: "concept_entry_plug", to: "eva_unit00", shortcodes: ["entryPlug", "unit00"], notes: "Rei rides an entry plug into Unit-00 --- the capsule slots into the spine and floods with LCL on activation." },
    { from: "concept_entry_plug", to: "eva_unit02", shortcodes: ["entryPlug", "unit02"], notes: "Asuka's entry plug slots into Unit-02's spine and floods with LCL --- the same capsule every unit uses.", revealedAt: { kind: "ep", episode: 8 }, revealedAtSource: "Inherits Unit-02's Ep. 8 first-appearance gate" },
    { from: "concept_berserk", to: "char_shinji", shortcodes: ["berserk", "shinji"], notes: "When Unit-01 goes berserk Shinji is a passenger --- blacked out or dissolved into LCL while the unit fights on its own.", revealedAt: { kind: "ep", episode: 2 }, revealedAtSource: "https://wiki.evageeks.org/Berserk --- Unit-01 acts without Shinji's input during its berserk awakenings" },
    { from: "concept_berserk", to: "char_yui", shortcodes: ["berserk", "yui"], notes: "Berserk mode is Yui's soul in Unit-01 finally moving on its own --- the mother in the machine deciding she has had enough.", revealedAt: { kind: "ep", episode: 20 }, revealedAtSource: "https://wiki.evageeks.org/Berserk --- the will inside Unit-01 (Yui, per the Ep. 20 contact-experiment reveal) drives the berserk state" },
    { from: "concept_dummy_plug", to: "char_gendo", shortcodes: ["dummyPlug", "gendo"], notes: "Gendo orders the Dummy Plug to seize Unit-01 from Shinji and crush the possessed Unit-03 --- Toji still aboard (Ep. 18).", revealedAt: { kind: "ep", episode: 18 }, revealedAtSource: "https://wiki.evageeks.org/Dummy_Plug --- 'Gendo has it seize control of Eva-01 from Shinji in Episode 18'" },
    { from: "concept_s2_engine", to: "angel_01_adam", shortcodes: ["s2Engine", "adam"], notes: "Every Angel runs on an S2 Engine --- the inexhaustible core that traces back to Adam, the progenitor of the Angels.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/S2_Engine --- the S2 Engine is the Angels' power organ, the line that descends from Adam" },

    // --- Lance of Longinus / two-Moons / Impact backstory. ---
    { from: "concept_lance_of_longinus", to: "loc_antarctica", shortcodes: ["lanceOfLonginus", "antarctica"], notes: "The Lance is shipped up from the Antarctic and first appears under wraps on the carrier deck (Ep. 12).", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/Lance_of_Longinus --- 'transported from the Antarctic to Nerv' (origin shown in the Ep. 21 Katsuragi-Expedition backstory)" },
    { from: "concept_black_moon", to: "angel_02_lilith", shortcodes: ["blackMoon", "lilith"], notes: "Lilith arrived inside the Black Moon buried under Tokyo-3 --- the Geofront is the egg's hollow upper shell.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Black_Moon --- the Black Moon is Lilith's vessel beneath the Geofront" },
    { from: "event_second_impact", to: "concept_white_moon", shortcodes: ["secondImpact", "whiteMoon"], notes: "Second Impact was the cracking of the White Moon --- the Katsuragi Expedition's contact experiment with the Adam it held.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/White_Moon --- Adam's White Moon vessel is breached at Second Impact" },
    { from: "event_first_impact", to: "concept_white_moon", shortcodes: ["firstImpact", "whiteMoon"], notes: "First Impact, the giant collision that formed the Moon, stranded the White Moon and its passenger Adam in Antarctica.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "https://wiki.evageeks.org/First_Impact --- the Giant Impact strands the White Moon (Adam's vessel) on Earth" },
    { from: "event_first_impact", to: "loc_antarctica", shortcodes: ["firstImpact", "antarctica"], notes: "First Impact left the White Moon buried in what became Antarctica --- the seed of everything the show turns on.", revealedAt: { kind: "ep", episode: 21 }, revealedAtSource: "Inherits First Impact + Antarctica's Ep. 21 backstory gates" },

    // --- Angel hunts and pilot ops: Operation Yashima's two shooters, the
    // Mt. Asama dive, and the late-show Terminal Dogma convergence. ---
    { from: "event_operation_yashima", to: "char_shinji", shortcodes: ["operationYashima", "shinji"], notes: "Shinji takes the single positron-rifle shot at the end of Operation Yashima, eyes shut, on Misato's mark (Ep. 6).", revealedAt: { kind: "ep", episode: 6 }, revealedAtSource: "https://wiki.evageeks.org/Operation_Yashima --- Shinji fires the positron rifle to kill Ramiel (Ep. 6)" },
    { from: "event_operation_yashima", to: "char_rei", shortcodes: ["operationYashima", "rei"], notes: "Rei holds the A.T.-Field heat shield in Unit-00 so Shinji's rifle survives Ramiel's counter-beam (Ep. 6).", revealedAt: { kind: "ep", episode: 6 }, revealedAtSource: "https://wiki.evageeks.org/Operation_Yashima --- Rei in Unit-00 holds the shield during the operation (Ep. 6)" },
    { from: "char_asuka", to: "loc_mt_asama", shortcodes: ["asuka", "mtAsama"], notes: "Asuka abseils into the Mt. Asama caldera in heat-resistant Type-D armor to pull the embryonic Sandalphon out of the magma (Ep. 10).", revealedAt: { kind: "ep", episode: 10 }, revealedAtSource: "https://wiki.evageeks.org/Sandalphon --- Asuka in Unit-02 descends into the Mt. Asama caldera in Ep. 10" },
    { from: "eva_unit02", to: "loc_mt_asama", shortcodes: ["unit02", "mtAsama"], notes: "Unit-02 in Type-D equipment is lowered into the Mt. Asama caldera for the Sandalphon retrieval (Ep. 10).", revealedAt: { kind: "ep", episode: 10 }, revealedAtSource: "https://wiki.evageeks.org/Sandalphon --- Unit-02 in Type-D armor enters the caldera in Ep. 10" },
    { from: "char_toji", to: "loc_matsushiro", shortcodes: ["toji", "matsushiro"], notes: "Toji is sealed inside Unit-03's entry plug for the activation test at Matsushiro --- the test Bardiel hijacks (Ep. 18).", revealedAt: { kind: "ep", episode: 18 }, revealedAtSource: "https://wiki.evageeks.org/Matsushiro --- Unit-03's activation test with Toji aboard happens at Matsushiro (Ep. 18)" },
    { from: "char_rei", to: "loc_terminal_dogma", shortcodes: ["rei", "terminalDogma"], notes: "Rei descends to Terminal Dogma to retrieve the Lance --- and later merges with the Lilith crucified there (Ep. 23 / EoE).", revealedAt: { kind: "ep", episode: 23 }, revealedAtSource: "https://wiki.evageeks.org/Rei_Ayanami --- Rei retrieves the Lance from Terminal Dogma and ultimately merges with Lilith there" },
    { from: "char_kaworu", to: "loc_terminal_dogma", shortcodes: ["kaworu", "terminalDogma"], notes: "Kaworu walks Unit-02 down to Terminal Dogma believing he will reach Adam --- finds Lilith instead and asks to be killed (Ep. 24).", revealedAt: { kind: "ep", episode: 24 }, revealedAtSource: "https://wiki.evageeks.org/Kaworu_Nagisa --- Kaworu's Ep. 24 descent to Terminal Dogma" },
    { from: "char_kaworu", to: "angel_18_lilim", shortcodes: ["kaworu", "lilim"], notes: "Tabris, wearing Kaworu's face, is the one who names humanity 'Lilim' --- the line the show flinches at (Ep. 24).", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Lilim --- Kaworu / Tabris coins the term 'Lilim' for humanity in Ep. 24" },

    // --- JSSDF / Marduk: the finale assault and the Children-selection front. ---
    { from: "org_jssdf", to: "char_misato", shortcodes: ["jssdf", "misato"], notes: "Misato is gunned down by JSSDF troops storming NERV in End of Evangelion, after getting Shinji to the plug.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/Misato_Katsuragi --- Misato is shot by JSSDF forces during the End of Evangelion assault" },
    { from: "org_jssdf", to: "org_nerv", shortcodes: ["jssdf", "nerv"], notes: "The JSSDF storms NERV in End of Evangelion on SEELE's order --- the slaughter that kills most of the staff.", revealedAt: { kind: "eoe" }, revealedAtSource: "https://wiki.evageeks.org/JSSDF --- the JSSDF is deployed against NERV in End of Evangelion" },
    { from: "org_marduk", to: "char_shinji", shortcodes: ["marduk", "shinji"], notes: "The Marduk Institute is credited with 'selecting' Shinji as the Third Child --- a front for NERV's own choices.", revealedAt: { kind: "ep", episode: 4 }, revealedAtSource: "https://wiki.evageeks.org/Marduk_Institute --- the Institute that selects the Children for the EVA program" },
  ];
