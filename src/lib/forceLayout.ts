...
export interface LayoutOptions {
  ...
  centerForce?: number;
  /**
   * Optional node id to pin at the origin. When set, the pinned node's
   * position is locked to (0,0,0) every step and the per-step recenter
   * shifts everything by the pinned node's position (which is always zero,
   * so other nodes float relative to a fixed pivot instead of the moving
   * centroid). The renderer uses this so Shinji sits at the orb's center
   * --- the camera and sceneGroup already pivot on world origin, so a
   * pinned Shinji becomes the literal axis of rotation.
   */
  pinnedNodeId?: string;
...
const DEFAULT_OPTS: Required<
  Omit<LayoutOptions, "springLengthByKind" | "pinnedNodeId">
> & {
  springLengthByKind: Partial<Record<EdgeKind, number>>;
...
export function forceLayout3D(
  ...
  const pinnedId = opts.pinnedNodeId;

  // Initial random sphere placement. The pinned node (if any) starts at
  // origin and never moves --- every other node settles around it under
  // edge spring + center-pull pressure.
  ...
  for (let step = 0; step < opts.iterations; step++) {
    ...
    // Integrate. The pinned node (if any) is held at origin: zero velocity,
    // zero displacement. Every other node integrates as normal.
    for (const n of nodes) {
      if (n.id === pinnedId) {
    ...
    // Re-center each step so the layout doesn't drift. When a node is
    // pinned, recenter on ITS position (always 0,0,0 by definition) so the
    // pinned node remains the fixed pivot and everything else floats
    // relative to it. With no pin, recenter on the centroid as before so
    // disconnected components don't wander off.
    let cx = 0,
      cy = 0,
      cz = 0;
    if (pinnedId !== null && positions.has(pinnedId)) {
